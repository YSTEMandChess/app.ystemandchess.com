  ...
  socket.on("newGame", (msg) => {
    ...
    ongoingGames.forEach((element) => {
      if (
        ...
        console.log(
          "A user has requested that we make a new game. However I belive that there is a game already in progress. Sending them the new game state."
        );
    ...
    if (newGame) {
      console.log(
        `Creating game between: ${parsedmsg.mentor} and ${parsedmsg.student}`
      );
      let colors = [];
      if (Math.random() > 0.5) {
        colors = ["black", "white"];
